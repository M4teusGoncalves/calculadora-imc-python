...
janela = Tk()
janela.title('')
janela.geometry('295x230') #dimensões da janela
janela.configure(bg=co0) #cor de fundo da janela


# -------- dividindo a JANELA em duas partes -------- #

frame_cima= Frame(janela, width=295, height=50, bg=co0, pady=0, padx=0, relief='flat')
frame_cima.grid(row=0, column=0, stick=NSEW)
...
def calcular():
    ...
    if resultado == 18.5 or resultado < 18.5:
...
l_resultado = Label(frame_baixo, text='-', width=5, height=1, padx=6, pady=12, relief='flat', anchor='center', font=('Ivy 25 bold'), bg=co2, fg=co0)
l_resultado.place(x=175, y=10)


l_resultado_texto = Label(frame_baixo, text='', width=37, height=1, pady=12, padx=0, relief='flat', anchor='center', font=('Ivy 10 bold'), bg=co0, fg=co1)
